...
sProcessMode	=	Trim(Request.Form("hid_ProcessMode"))
iRecIDTrdRel			=	Trim(Request.Form("hid_TrdRelId"))
...
select case ucase(trim(sProcessMode)) 
case "NEW"
	bnew = true
	
	iParentDocID = 0
	sPONo = trim(Request.Form("BIG_04"))
	sInvoiceNo=trim(Request.Form("BIG_02"))	
	ProcessName="NEW"
...
case "FROMPO"
	...
	iParentDocID = iRecIdDoc
	sPONo = trim(Request.Form("BIG_04"))
	sInvoiceNo=trim(Request.Form("BIG_02"))
	ProcessName="FROMPO"
end select

Dim sSQL, sISA15, sGS3, sISA7, sISA8, sGS2, sISANumber, sISA5, sISA6,sISA9,sISA10
	set oConn=server.CreateObject("adodb.connection")
		oConn.open(Str_ConnectionString_EdiFoundation)
		
''Fetching ICN,TSCN.FGCN Value
		set oCmdProcess=server.CreateObject("adodb.command")
			oCmdProcess.ActiveConnection=oconn
			oCmdProcess.CommandType =adCmdStoredProc
			oCmdProcess.CommandText="sp_GetUpdated_ICN_FGCN_TSCN"
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDTrdRel",adInteger,adParamInput,,iRecIDTrdRel)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ICN",adVarChar,adParamOutput,50)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("FGCN",adVarChar,adParamOutput,50)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("TSCN",adVarChar,adParamOutput,50)			
			oCmdProcess.Execute ,,adExecuteNoRecords
			sICN=oCmdProcess.parameters("ICN")
			sFGCN=oCmdProcess.parameters("FGCN")
			sTSCN=oCmdProcess.parameters("TSCN")
		set oCmdProcess=nothing	
''Fetching ISA,GS Value		
		
		set oCmdProcess=server.CreateObject("adodb.command")
			oCmdProcess.ActiveConnection=oconn
			oCmdProcess.CommandType =adCmdStoredProc
			oCmdProcess.CommandText="sp_OutBox_CommunicationDetails"
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDTrdRel",adInteger,adParamInput,,iRecIDTrdRel)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("FileCode",adVarChar,adParamInput,50,"810")
		set RsProcess=oCmdProcess.Execute 	
			if not RsProcess.eof then
				
				sISA15	= trim(RsProcess("SenderISA15"))
				sGS2	= trim(RsProcess("GSSenderID"))
				sISA5	= trim(RsProcess("SenderIDQualifier"))
				sISA6	= trim(RsProcess("SenderID"))
				sGS3	= trim(RsProcess("GSReceiverID"))
				sISA7	= trim(RsProcess("ReceiverIDQualifier"))
				sISA8	= trim(RsProcess("ReceiverID"))
				sISA9	= trim(RsProcess("ISA09"))
				sISA10	= trim(RsProcess("ISA10"))
			else
				response.Write "<b><font color=red>Communication ID's not defined !</font></b>"
				Response.End 
			end if
		set oCmdProcess=nothing
	...
	if Err.number <> 0 then
		sErrorMsg = sErrorMsg & vbcrlf & "ICN :-" & Err.number & "-" & Err.description & "<BR>"
		err.Clear 
	end if
...
s810Xml = s810Xml & "<SegmentRef Pos='0' ID='ISA'  Description='Interchange Control Header'>"
	...
	s810Xml = s810Xml & "<Element Pos='01' ID='479' Description='Functional Identifier Code' Type='ID' MinLength='2' MaxLength='2' Value='IN' /> "
...
''''''***************************************************************************************
if Err.number	<> 0 Then 
	sErrorMsg		=	sErrorMsg & "Error In XML String : "&Err.Description 
	Err.Clear
End if
...
'@ProcessName as varchar(20),	--NEW or EDIT or FROMPO
'@RecIDDoc as int,				--0
'@InvoiceNo as varchar(20),		--Invoice no or blank
'@RecIDTrdRel as int,			--RecIDTrdRel as  from tbl_TradeRelationShip table , sInvoiceNo
'@ParentIDDoc as int,			--ParentIDDoc as from tbl_DocumentData or 0
'@PONo as varchar(20),			--Purchase Order No from BEG_324 / 850 data ,TransDocNo
'@XMLData as Text,				--s810Xml
'@FileCode as varchar(50),		--810
'@RecIDUserLog as int 			--
...
if bNew or bFromPO then
	set oCmdProcess=server.CreateObject("adodb.command")
			oCmdProcess.ActiveConnection=oconn
			oCmdProcess.CommandType =adCmdStoredProc
			oCmdProcess.CommandText="sp_Save810Data"			
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ProcessName",adVarChar,adParamInput,20,ProcessName)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDDoc",adInteger,adParamInput,,0)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("InvoiceNo",adVarChar,adParamInput,20,sInvoiceNo)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDTrdRel",adInteger,adParamInput,,iRecIDTrdRel)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ParentIDDoc",adInteger,adParamInput,,iParentDocID)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("PONo",adVarChar,adParamInput,20,sPONo)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("XMLData",adLongVarChar,adParamInput,len(s810Xml)+10,s810Xml)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("FileCode",adVarChar,adParamInput,50,"810")
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDUserLog",adInteger,adParamInput,,Session("LoginId"))
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ICN",adVarChar,adParamInput,50,sICN)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("FGCN",adVarChar,adParamInput,50,sFGCN)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("TSCN",adVarChar,adParamInput,50,sTSCN)
			oCmdProcess.execute ,,adExecuteNoRecords
			set oCmdProcess=nothing
end if
...
if bEdit then
		set oCmdProcess=server.CreateObject("adodb.command")
			...
			oCmdProcess.CommandText="sp_Save810Data"			
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ProcessName",adVarChar,adParamInput,20,ProcessName)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDDoc",adInteger,adParamInput,,iRecIdDoc)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("InvoiceNo",adVarChar,adParamInput,20,sInvoiceNo)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("RecIDTrdRel",adInteger,adParamInput,,iRecIDTrdRel)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("ParentIDDoc",adInteger,adParamInput,,iParentDocID)
			oCmdProcess.Parameters.Append=oCmdProcess.CreateParameter("PONo",adVarChar,adParamInput,20,sPONo)
...
<TABLE width="100%" >
	<TR>
		...
		<%
		if trim(sDBErrors) = "" and trim(sErrorMsg) = "" then
			Response.Write "<Font size=3><b>Invoice is Successfully Placed!</b></Font>" & "<BR>"
		else
			Response.Write "<Font size=3>Invoice couldnot be successfully placed.</Font>"
			if Trim(sErrorMsg) <> "" then
				Response.Write "Processing Errors :- " & sErrorMsg
			end if
			if trim(sDBErrors) <> "" then
				Response.Write "Database Errors :- " & sDBErrors
			end if			
		end if 
		%>
		</TD>
	...
	<TR>
		<TD width="100%" Align=center><Input type=button value="Ok"  class=inputbutton style="Width:60" onclick="window.location.href = '/Edi/LiveEdi/Outbox.asp'" id=button1 name=button1></TD>
	</TR>
<TABLE>
</Center>
<BR><BR>
